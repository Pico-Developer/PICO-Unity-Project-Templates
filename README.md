...
## 模板目录 / Template Directories

| 目录 / Directory | 模式 / Mode | 说明 / Description |
| --- | --- | --- |
| `PICOXR` | PICO XR | 基于 PICO XR 模式的工程模板，适合直接使用 PICO Unity SDK / PICO XR 能力进行开发。<br>Project template based on the PICO XR mode. Use this when developing directly with PICO Unity SDK / PICO XR features. |
| `OpenXR` | Unity OpenXR | 基于 Unity OpenXR 模式的工程模板，适合使用 Unity OpenXR 工作流并结合 PICO Unity SDK 能力进行开发。<br>Project template based on the Unity OpenXR mode. Use this when following the Unity OpenXR workflow while integrating PICO Unity SDK features. |
...
1. 安装 Unity `6000.0.73f1`，并确保已安装 Android Build Support。
   Install Unity `6000.0.73f1` and make sure Android Build Support is installed.
2. 根据需要选择模板目录：
   Choose the template directory based on your integration mode:
...
3. 在 Unity Hub 中选择 `Add project from disk`，然后选择对应的模板目录。
   In Unity Hub, select `Add project from disk`, then choose the corresponding template directory.
4. 等待 Unity Package Manager 完成依赖解析和资源导入。
   Wait for Unity Package Manager to resolve dependencies and import assets.
5. 在 Unity 中切换到 Android 平台后进行开发、调试和构建。
   Switch the project to the Android platform in Unity before development, debugging, and building.
